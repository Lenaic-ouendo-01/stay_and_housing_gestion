...
<head>
    ...
    <title>Liste cluster</title>
</head>
<body>
    <div class="flex justify-between mx-10 mt-5">
        <p class="text-3xl font-fam2 font-bold flex justify-center text-blue-700 mt-5">Liste des Logements</p>
        <div class="flex justify-between gap-5">
            ...
            <a href="{{ route('logement') }}" class="p-2 border-2 rounded-xl m-2 font-fam1 bg-sky-600 text-white">+ New Housing</a>
        </div>
    </div>
    ...
    <main class="text-xl mt-6 font-medium">
        <table  class="rounded border mt-10 tableau mx-auto">
            <tr class="rounded border">
                <th class="rounded border-r-2 px-3 bg-gray-500/40">Code</th>
                <th class="rounded border-r-2 px-3 bg-gray-500/40">Nom</th>
                <th class="rounded border-r-2 px-3 bg-gray-500/40">Capacité</th>
                <th class="rounded border-r-2 px-3 bg-gray-500/40">Type</th>
                <th class="rounded border-r-2 px-3 bg-gray-500/40">Lieu</th>
                <th class="rounded border-r-2 px-3 bg-gray-500/40">Photo</th>
                <th class="rounded border-r-2 px-3 bg-gray-500/40">Disponibilité</th>
                <th class="rounded border-r-2 px-3 bg-gray-500/40">Action</th>
            </tr>
            @foreach ($logements as $logement)
                <tr class="rounded border">
                    <td class="rounded border-r-2 px-3">{{ $logement->code }}</td>  
                    <td class="rounded border-r-2 px-3">{{ $logement->nom }}</td>
                    <td class="rounded border-r-2 px-3">{{ $logement->capacite }}</td>
                    <td class="rounded border-r-2 px-3">{{ $logement->type }}</td>
                    <td class="rounded border-r-2 px-3">{{ $logement->lieu }}</td>
                    <td class="rounded border-r-2 p-3">
                        <img src="{{ asset('uploads/logements/'.$logement->photo) }}" class="rounded-full w-20 h-20" alt="">
                    </td>
                    <td class="rounded border-r-2 px-3">{{ $logement->disponible }}</td>
                    <td class="rounded px-3 py-6 flex gap-2">
                        <a href="{{ route('logement.edit', ['id' => $logement->id]) }}" class="p-1 border-2 rounded-xl m-2 bg-teal-800 text-white">Modifier</a>
                        <form action="{{ route('logement.delete', ['id' => $logement->id]) }}" id="form" method="post">
                            @csrf
                            @method('delete')
...
form.addEventListener('submit', (e)=>{
    debugger
    e.preventDefault()
    const canDelete = confirm('Voulez vous vraiment supprimer ce logement ?')
    if(canDelete){
        e.target.submit()
    }
})
