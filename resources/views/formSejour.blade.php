...
<head>
    ...
    <title>Form of stay</title>
</head>
<body class="bg-blue-300/50">

    <div class="">
        <header class="text-3xl font-fam2 font-bold flex justify-center text-blue-700 mt-5">Nouveau Séjour</header>
        ...
        <main class="text-xl mt-6 font-fam1 font-medium flex justify-center">
            <form action="{{ route('sejour.store') }}" method="POST">
                ...
                <div class="flex mt-4 justify-between">
                    ...
                    <select class="w-1/2 ml-5 lg:text-base text-xs text-black/80 border-2 border-blue-900 rounded-xl p-1" name="voyageur_id" id="">
                        @foreach ($voyageurs as $voyageur)
                            <option value="{{$voyageur->id}}">{{ $voyageur->prenom }} {{ $voyageur->nom }}</option>
                        @endforeach
                    </select>
                ...
                <div class="flex mt-4 justify-between">
                    ...
                    <input class="w-1/2 ml-5 lg:text-base text-xs text-black/80 border-2 border-blue-900 rounded-xl p-1" type="date" name="debut" id="">
                </div>

                <div class="flex mt-4 justify-between">
                    <label for="">Fin : </label>
                    <input class="w-1/2 ml-5 lg:text-base text-xs text-black/80 border-2 border-blue-900 rounded-xl p-1" type="date" name="fin" id="">
                </div>
